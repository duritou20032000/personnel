...
export default class RlsFilesignDetail extends PureComponent{
    ...
    render(){
        ...
        const signerlist = curritem.signer.map((item,index) => {

            let pass = item.pass!=3?item.pass!=2?"同意":"拒绝":"待签名";

            return (
                <div  key={index} className={styles.signerPanel}>
                    <div className={styles.headCell}>
                        <img src={item.portrait}/>
                        <div>{item.name}</div>
                    </div>
                    <div className={styles.contentCell}>
                        <div style={{color:item.pass!=3?item.pass==1?'#52c41a':'#f5222d':'#1890ff'}}>{pass}</div>
                        <div>数字签名:{item.sign}</div>
